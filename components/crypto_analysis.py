...
import requests
from utils.constants import crypto_ticker_to_company_dict 
import nltk
...
crypto_ticker_to_company = {ticker: crypto_ticker_to_company_dict.get(ticker, ticker) for ticker in crypto_ticker_to_company_dict.keys()}

# Convert the ticker_to_company dictionary to a list of company names
company_names = list(crypto_ticker_to_company.values())
...
# Main Streamlit App
def crypto_analysis_app():
    st.sidebar.subheader("Crypto Analysis")
    ...
    ticker = [ticker for ticker, company in crypto_ticker_to_company.items() if company == selected_company][0]
