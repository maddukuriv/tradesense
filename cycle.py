...
view_option = st.sidebar.radio("View Option", ["Stock Analysis", "Analysis Summary"])
...
for ticker in tickers:
    ...
    if not data.empty:
        ...
        # Plotting for individual stock analysis
        if view_option == "Stock Analysis" and ticker == tickers[0]:
            ...
            fig, ax = plt.subplots()
            df['Close'].plot(ax=ax, title=f"{ticker} Close Price")
            df['SMA_10'].plot(ax=ax, label='SMA 10')
            df['SMA_20'].plot(ax=ax, label='SMA 20')
            df['SMA_50'].plot(ax=ax, label='SMA 50')
            ax.legend()
            st.pyplot(fig)
...
if view_option == "Analysis Summary":
