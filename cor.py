...
def calculate_scores(data):
    # Your existing calculate_scores function goes here
    pass

# Function to calculate the correlation between Close and Technical Indicators
def calculate_correlation(df):
    indicators = df.columns[df.columns != 'Close']
    correlations = df[['Close'] + list(indicators)].corr()
    return correlations['Close'].drop('Close')
...
if ticker:
    ...
    st.subheader(f"Correlation between Close and Technical Indicators for {ticker}")
    st.write(correlations)

    # Display data and charts if needed
    st.subheader("Stock Data")
    st.line_chart(data['Close'])

    st.subheader("Technical Indicators")
    st.line_chart(data.drop(columns=['Close']))
