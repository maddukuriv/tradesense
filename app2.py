...
if not st.session_state.logged_in:
    ...
    if choice:
        if choice == "Markets":
            ...
            # Generate insights
            second_query_df = first_query_df[(first_query_df['RSI'] < 70) & (first_query_df['RSI'] > 55) & (first_query_df['ADX'] > 20)& (first_query_df['MACD'] > 0)]
            ...
            if selected_stock:
                def load_data(ticker, start, end):
                    ...
                    # Buy/Sell Signal Generation (Improved Strategy)
                    def generate_signals(prices, reconstructed_signal, amplitude_envelope, instantaneous_frequency, df):
                        ...
                        for i in range(2, len(prices) - 1):
                            if (reconstructed_signal[i] > reconstructed_signal[i - 1] and
                                    ...
                                    df['MACD'][i] > df['MACD_Signal'][i] and df['ADX'][i] > 20 and 55 < df['RSI'][
                                        i] < 70):
                    ...
                    # Generate Signals
                    buy_signals, sell_signals = generate_signals(prices, reconstructed_signal, amplitude_envelope,
                    ...
                    signals_fig.add_trace(
                        go.Scatter(x=[dates[sig[0]] for sig in buy_signals], y=[sig[1] for sig in buy_signals],
                                   mode='markers', marker=dict(color='green', size=10), name='Buy Signal'))

                    # Sell signals
                    signals_fig.add_trace(
                        go.Scatter(x=[dates[sig[0]] for sig in sell_signals], y=[sig[1] for sig in sell_signals],
                ...
                # Define the default date range for the past month
            ...
            if df.empty:
                ...
                features = ['Open', 'High', 'Low', 'Volume', 'SMA_5', 'SMA_10', 'SMA_20', 'MACD', 'MACD_Signal', 'RSI',
                            'BB_High', 'BB_Low', '5_DAYS_STD_DEV', 'ATR', 'Volume_MA_20', 'Volume_MA_10']
                X = data[features]
                y = data['Close']
                ...
                st.text("Training ARIMA/SARIMA model...")
                ...
                lstm_fig.add_trace(go.Scatter(x=data.index[-len(y_test):], y=y_pred_lstm.flatten(), mode='lines',
